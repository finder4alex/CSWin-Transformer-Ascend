...
def cosine_lr(args, batch_num):
    ...
    for epoch in range(args.epochs - args.cool_length - args.tail_length - args.over_length):
        for batch in range(batch_num):
            learning_rate.append(_lr_adjuster(epoch + batch / batch_num))
    ...
    for epoch in range(args.tail_length):
        for _ in range(batch_num):
            learning_rate.append(args.tail_lr)

    for epoch in range(args.over_length):
        for _ in range(batch_num):
            learning_rate.append(args.over_lr)

    # learning_rate = np.clip(learning_rate, args.min_lr, max(learning_rate))
    min_lrs = [args.base_lr, args.min_lr, args.warmup_lr, args.cool_lr, args.tail_lr, args.over_lr]
    min_lr = min(min_lrs)
    learning_rate = np.clip(learning_rate, min_lr, max(learning_rate))
    return learning_rate
